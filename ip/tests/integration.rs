...
fn missing_input_file_exits_with_error() {
    let out = ip()
        .args([
            ...
            "-o",
            &tmp_output("ip_test_dummy.png"),
            "-p",
            "mirror",
            "-d",
            &config("mirror_h.json"),
            "-l",
            &plugin_dir(),
        ])
        .output()
...
fn missing_plugin_dir_exits_with_error() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("plugin directory not found"),
        "stderr: {stderr}"
    );
}
